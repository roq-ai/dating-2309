...
import { getLocationPreferenceById, updateLocationPreferenceById } from 'apiSdk/location-preferences';
...
function LocationPreferenceEditPage() {
  ...
  const handleSubmit = async (values: LocationPreferenceInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateLocationPreferenceById(id, values);
      mutate(updated);
      resetForm();
