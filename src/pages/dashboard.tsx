...
import { getLocationPreferences } from 'apiSdk/location-preferences';
...
import { MatchListPage as ShowCaseList } from 'pages/matches';

function DashboardPage() {
  const { session } = useSession();

  const socialMediaStat = useSWR<PaginatedInterface<SocialMediaInterface>>(
    () => `/social-medias`,
    () => getSocialMedias({ limit: 1, offset: 0 }),
  );

  const locationPreferenceStat = useSWR<PaginatedInterface<LocationPreferenceInterface>>(
    () => `/location-preferences`,
    () => getLocationPreferences({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Social Medias',
      result: socialMediaStat?.data?.totalCount,
      isLoading: socialMediaStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Location Preferences',
      result: locationPreferenceStat?.data?.totalCount,
      isLoading: locationPreferenceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Verifications',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`matches`} passHref legacyBehavior>
                  <Button
