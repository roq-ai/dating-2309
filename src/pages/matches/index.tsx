...
export function MatchListPage(props: MatchListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getMatches({
        relations: ['user_match_matched_user_idTouser', 'user_match_user_idTouser'],
  ...
  const columns: ColumnType[] = [
    { id: 'match_score', header: 'match_score', accessorKey: 'match_score' },
    { id: 'match_status', header: 'Match Status', accessorKey: 'match_status' },
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_match_matched_user_idTouser',
          header: 'User Match Matched User Id Touser',
          accessorKey: 'user_match_matched_user_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_match_matched_user_idTouser?.id}`}
            >
              {record.user_match_matched_user_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_match_user_idTouser',
          header: 'User Match User Id Touser',
          accessorKey: 'user_match_user_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_match_user_idTouser?.id}`}
            >
              {record.user_match_user_idTouser?.email}
            </Link>
          ),
        }
      : null,
